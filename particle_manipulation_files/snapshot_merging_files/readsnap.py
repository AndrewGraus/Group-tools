...
##    print {fname,fname_base,fname_ext}
    ...
    print "flacg_cooling:",flag_cooling
...
def check_if_filename_exists(sdir,snum,snapshot_name='snapshot_440',extension='.hdf5',four_char=0):
    for extension_touse in [extension,'.bin','']:
##        fname=sdir+'/'+snapshot_name+'_'
##        ext='00'+str(snum);
        fname=sdir+'/'+snapshot_name##+'.' Modified by AlexGlez. 10/19/2015
        ext='.'+str(snum);        
        ...
        fname_base=fname
        #modificado por Alejandro Glez. 10/19/2015
        if (snum>0): ext='.'+str(snum)
        fname+=ext
        ...
        fname=fname_base+ext+extension_touse; ##Modified by AlexGlez. 10/19/2015
