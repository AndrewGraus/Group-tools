...
import struct,sys
from numpy import fromstring,array,append,empty
...
if __name__ == "__main__":
    ...
    #scratch that again--here, I want to leave gas, halo, and stars unchanged, but split boundary into disk, bulge, then the most massive 2 into boundary
    ...
    if len(masses) != 4:
        print "There are {0} different masses in the boundary group; this script was hardcoded for exactly 4.  Do it by hand.".format(len(masses))
        sys.exit(1337)

    level10mass = masses[0]
    level9mass = masses[1]
    level8mass = masses[2]
    level7mass = masses[3]

    diskpos = allbndrypos[allbndrymass == level10mass]
    diskvel = allbndryvel[allbndrymass == level10mass]
    diskID = allbndryid[allbndrymass == level10mass]
    ...
    #scrizatch--bulge is just level 9, then boundary is 8 and 7, which means I don't need a mass block
    
    bulgepos = allbndrypos[allbndrymass == level9mass]
    bulgevel = allbndryvel[allbndrymass == level9mass]
    bulgeID = allbndryid[allbndrymass == level9mass]
    ...
    print "Placed {0} particles in the disk group with a mass of {1}".format(ndisk,level10mass)
    print "Placed {0} particles in the bulge group with a masss of {1}.".format(ndisk,level9mass)
    ...
    f.write(struct.pack('<6d',header[1][0],header[1][1],level10mass,level9mass,header[1][4],0)) #Mass table 
